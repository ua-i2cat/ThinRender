...
#include "DirectionalLightTexturedShadowMapShader.h"

DirectionalLightTexturedShadowMapShader::DirectionalLightTexturedShadowMapShader(){
	texture = 0;
	hasNormals = true;
	hasUV = true;
	hasTangents = false;
	hasBitangents = false;
	loadShader("DirectionalLightTexturedShadowMapShader");
}

void DirectionalLightTexturedShadowMapShader::loadVars(){
...
void DirectionalLightTexturedShadowMapShader::setVars(glm::mat4 projView, glm::mat4 model, glm::mat4 view, glm::mat4 projection, Light* renderLight, glm::vec4 lightWorldPosition,glm::vec4 light_direction, glm::vec3 light_ambient, glm::mat4 projectionLight, glm::mat4 viewLight, GLuint depthTexture, bool receiveShadows){
	...
	glUniform1i(uniformTextureDepth, /*GL_TEXTURE*/1);
}

void DirectionalLightTexturedShadowMapShader::loadTexture(Texture* texture){
	if(texture){
