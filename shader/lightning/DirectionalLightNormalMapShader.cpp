...
#include "DirectionalLightNormalMapShader.h"

DirectionalLightNormalMapShader::DirectionalLightNormalMapShader(){
	texture = 0;
	normalMapTexture = 0;
	hasNormals = true;
	hasTangents = true;
	hasBitangents = true;
	hasUV = true;
	loadShader("DirectionalLightNormalMapShader");
}
...
void DirectionalLightNormalMapShader::setVars(glm::mat4 projView, glm::mat4 model, glm::mat4 view, glm::mat4 projection, Light* renderLight, glm::vec4 lightWorldPosition, glm::vec4 light_direction, glm::vec3 light_ambient, glm::mat4 projectionLight, glm::mat4 viewLight, GLuint depthTexture, bool receiveShadows){
...
void DirectionalLightNormalMapShader::loadTexture(Texture* texture){
	if(texture){
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texture->getTextureId());
		glUniform1i(uniformTexture, /*GL_TEXTURE*/0);
